import { createMutable } from "solid-js/store"
import { batch, createSignal, For, lazy, onMount, Show, Switch, Match as SwitchMatch, } from "solid-js"
import A from "@arksouthern/jsx/ax"
import HandleSet from "@arksouthern/jsx/hx"
import Variables from "@arksouthern/jsx/vx"
import { createApi } from "~/lib/url"
import { App } from "~/Types"
import { XpWindow } from "~/components/luna/window"
import { XpTitleButtons, XpTitleButtonsClose, XpTitleButtonsNormal } from "~/components/luna/title-buttons"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "~/components/ui/dropdown-menu"
import { mx, Mx } from "~/lib/mx"
import MatchAs from "@arksouthern/jsx/mx"
import VarSet from "@arksouthern/jsx/vx"
import { store, zIndex } from "~/Store"
import { Stats } from "fs"
import { api, FioStat, SaveLocation } from "../backend"
import { ContextMenu, ContextMenuContent, ContextMenuGroup, ContextMenuGroupLabel, ContextMenuItem, ContextMenuPortal, ContextMenuRadioGroup, ContextMenuRadioItem, ContextMenuSeparator, ContextMenuSub, ContextMenuSubContent, ContextMenuSubTrigger, ContextMenuTrigger } from "~/components/ui/context-menu"
import { HoverCard, HoverCardContent, HoverCardTrigger } from "~/components/ui/hover-card"
import { clipboardFsCopy, clipboardFsPaste, closeAppWindow, kbdShortcutOn, openApp, openFile, queryFileOpener } from "~/lib/luna"
import { XpDropDownItem, XpDropDownMenu, XpDropDownTrigger } from "~/components/luna/dropdown-menu.tsx"
import { XpRightClickMenu, XpRightClickMenuDivider, XpRightClickMenuItem } from "~/components/luna/right-click-menu"
import { XpBarMenu, XpBarMenuCheckboxItem, XpBarMenuDivider, XpBarMenuItem, XpBarMenuLineItem, XpBarMenuMasterItem } from "~/components/luna/bar-menu"
import { createAbout } from "~/components/luna/about"
import { XpAsideAccordion, XpAsideAccordionItem } from "~/components/luna/side-accordian"

const API = createApi<typeof api>("@arksouthern/luna.explore")

type SortMx = ({ as: "size" } | { as: "dateModified" } | { as: "dateCreated" } | { as: "name" }) & { direction: "up" | "down" }
type ViewIconListMx = { as: "detail" } | { as: "medium" } | { as: "filmstrip" } | { as: "tiles" } | { as: "list" } | { as: "thumbnails" }
type ViewSidebarMx = { as: "prompts" } | { as: "tree" } | { as: "favorites" } | { as: "none" }

export default function ProgFileExplorer(props: App) {

  const pathFirstLoad = () => {
    if (!props.app.params.openPath) return ""
    if (props.app.params.openPath.endsWith("/")) return props.app.params.openPath
    return `${props.app.params.openPath}/`
  }

  const self = createMutable({
    dir: pathFirstLoad(),
    sorts: [] as SortMx[],
    selectedItems: [] as number[],

    isViewStatusBar: true,
    isViewAddressBar: true,
    isViewButtonsBar: true,

    networkDirCurrentFiles: [] as FioStat[],
    networkFavoritesFiles: [] as FioStat[],
    networkTreeDir: [] as Awaited<ReturnType<typeof API['dirList']>>,

    viewIconListMx: { as: "detail" } as ViewIconListMx,
    viewSidebarMx: { as: "prompts" } as ViewSidebarMx,

    get getSortedFiles() {
      const copiedFiles = this.networkDirCurrentFiles.slice()
      for (const sort of this.sorts) {
        mx(sort)({
          dateCreated: () => {
            if (sort.direction == 'up') {
              copiedFiles.sort((a, b) => (a.birthtimeMs || 0) - (b.birthtimeMs || 0))
            } else {
              copiedFiles.sort((a, b) => (b.birthtimeMs || 0) - (a.birthtimeMs || 0))
            }
          },
          dateModified: () => {
            if (sort.direction == 'up') {
              copiedFiles.sort((a, b) => (a.mtimeMs || 0) - (b.mtimeMs || 0))
            } else {
              copiedFiles.sort((a, b) => (b.mtimeMs || 0) - (a.mtimeMs || 0))
            }
          },
          name: () => {
            if (sort.direction == 'up') {
              copiedFiles.sort((a, b) => a.name.localeCompare(b.name))
            } else {
              copiedFiles.sort((a, b) => b.name.localeCompare(a.name))
            }
          },
          size: () => {
            if (sort.direction == 'up') {
              copiedFiles.sort((a, b) => (a.size || 0) - (b.size || 0))
            } else {
              copiedFiles.sort((a, b) => (b.size || 0) - (a.size || 0))
            }
          },
        })(sort)
      }
      return copiedFiles
    }
  })

  async function updateDirThenCurrentFiles(props: { dir: string }) {
    const items = await API.folderRead(props)
    batch(() => {
      self.selectedItems.splice(0)
      self.dir = props.dir
      self.networkDirCurrentFiles = items
    })
  }

  onMount(async () => {
    if (self.dir) {
      updateDirThenCurrentFiles({ dir: self.dir })
      self.networkTreeDir = await API.dirList({ path: self.dir })
    } else {
      self.networkTreeDir = await API.dirList({ path: "/" })
    }
    self.networkFavoritesFiles = await API.dirList({ path: "../data/Favorites/ignore" })
  })

  function newPath(props: { path: string }) {
    return props.path.replaceAll("\\", "/")
  }

  function DirTree(props: { dirTree: Awaited<ReturnType<typeof API['dirList']>>, path: string }) {
    return (
      <>
        <ul class="text-[.62rem] flex-1 relative px-1 flex flex-col pl-[.8rem]">
          {/* <button class="absolute top-0.5 left-[17px] z-20 bg-[#f3f4f6] border-[#61646d] border w-3 h-3 scale-75"><span class="block -translate-y-1.5 ml-[1px]  text-sm">+</span></button> */}
          <For each={props.dirTree.filter(x => x.type.as == 'folder')}>{
            x => (
              <>
                <li
                  class="text-nowrap relative flex gap-1 items-center"
                >
                  <span onClick={async () => {
                    if (x.type.as != 'folder') return
                    if (x.type.fios.length)
                      x.type.fios = []
                    else
                      x.type.fios = await API.dirList({ path: `${props.path.split("/").slice(0, -1).join("/")}/${x.name}/ignore` })
                  }}>
                    <button class="absolute font-normal bg-[linear-gradient(135deg,#FFFFFF,#C1BAAC)] rounded-[1px] top-0.5 -left-[1rem] z-20 bg-[#f3f4f6] border-[#7F97B2] border w-2.5 h-2.5 scale-75">
                      {x.type.as == 'folder' && x.type.fios.length
                        ?
                        <span class="block leading-[1.2rem] -translate-y-1.5 ml-[0.5px]  text-[.62rem]">-</span>
                        :
                        <span class="block leading-[1.2rem] -translate-y-1.5 ml-[0.5px]  text-[.62rem]">+</span>
                      }
                    </button>

                    {x.type.as == 'folder' && x.type.fios.length
                      ?
                      <img class="h-4 w-4 min-w-4" src="/src/assets/shell32/5.ico" />
                      :
                      <img class="h-4 w-4 min-w-4" src="/src/assets/shell32/4.ico" />
                    }
                  </span>
                  <span
                    class="hover:text-[#00007B] hover:underline"
                    onClick={async () => {
                      if (x.type.as != 'folder') return
                      if (x.type.fios.length) {
                        x.type.fios = []
                      }
                      else {
                        x.type.fios = await API.dirList({ path: `${props.path.split("/").slice(0, -1).join("/")}/${x.name}/ignore` })
                        const dir = `${props.path.split("/").slice(0, -1).join("/")}/${x.name}`
                        updateDirThenCurrentFiles({ dir })
                      }
                    }}>
                    {x.name}
                  </span>
                </li>
                {x.type.as == 'folder' && x.type.fios.length &&
                  <A.SubTree>
                    {x.type.as == 'folder' &&
                      <DirTree dirTree={x.type.fios} path={`${newPath(props).split("/").slice(0, -1).join("/")}/${x.name}/ignore`} />
                    }
                  </A.SubTree>
                }
              </>
            )
          }</For>
        </ul>
      </>
    )
  }

  const [About, setAbout] = createAbout({ app: props.app, offsetX: 4, offsetY: 2, sizeX: 30, sizeY: 18 })

  return (
    <A.FileExplorer>
      <About icon={""} title={
        <>
          <A.TitleText class="flex-1 pointer-events-none pr-1 tracking-[.032rem] overflow-hidden whitespace-nowrap text-ellipsis">
            About Explorer
          </A.TitleText>
          <XpTitleButtons>
            <XpTitleButtonsClose {...props} onClick={() => setAbout.dialogHide()} />
          </XpTitleButtons>
        </>
      }>
        Replicating Microsoft's <small class="text-[0.75em]">&#174;</small> Explorer <br />
        Version 0.4.3 (Build 2025.03-06) <br />
        Arkansas Soft Construction <br />
        <br />
        <br />
        This product is licensed under the terms of the MIT <br />
        License Agreement to: <br />
        <br />
        &nbsp;&nbsp;&nbsp; Unknown User <br />
        &nbsp;&nbsp;&nbsp; Unknown Organization
      </About>
      <HandleSet
        handlers={{
          itemsCopy: async () => {
            const openPathList = self.selectedItems.map(id => `${self.dir}${self.networkDirCurrentFiles[id].name}`)
            await clipboardFsCopy({ openPathList })
          },
          itemsPaste: async () => {
            const result = await clipboardFsPaste({})
            if (result.as == "pasteFailed") return
            await Promise.all(
              result.openPathList.map(async (originalPath) => {
                await API.anyCopy({ originalPath, destPath: `${self.dir}${originalPath.split("/").pop()}` })
              })
            )
            updateDirThenCurrentFiles({ dir: self.dir })
          },
          favClick: (fav: FioStat) => async () => {
            self.selectedItems.splice(0)

            if (fav.type.as != "shortcut") return
            const { prog, params } = fav.type.shortcut
            if (prog == "@arksouthern/luna.explore")
              updateDirThenCurrentFiles({ dir: params.openPath })
            else
              openApp({ params, program: prog })
          },
          dirOpenProperties: async () => {
            openApp({ program: "@arksouthern/luna.explore.properties", params: { openPath: self.dir } })
          },
          navDirUp: async () => {
            let dir = self.dir.split("/") as any
            dir = dir.slice(0, -2)
            dir = dir.join("/") + "/"
            updateDirThenCurrentFiles({ dir })
          },
          sortsFileListToggle: (as: SortMx['as']) => () => {
            const sIndex = self.sorts.findIndex((s) => s.as == as)
            const sort = self.sorts[sIndex]
            if (!sort) self.sorts.push({ as, direction: "up" })
            else if (sort.direction == "up") sort.direction = "down"
            else self.sorts.splice(sIndex, 1)
          },
          appClose: async () => {
            closeAppWindow(props)
          },
          displaySidebar: (as: ViewSidebarMx['as']) => () => {
            self.viewSidebarMx = self.viewSidebarMx.as == as
              ? { as: "prompts" }
              : { as }
          },
          displayStandardButtonsToggle: () => {
            self.isViewButtonsBar = !self.isViewButtonsBar
          },
          displayAddressBarToggle: () => {
            self.isViewAddressBar = !self.isViewAddressBar
          },
          displayStatusBarToggle: () => {
            self.isViewStatusBar = !self.isViewStatusBar
          },
          anySelect: (props: { file: FioStat, index: number }) => (e: any) => {
            if (!(e.ctrlKey || e.metaKey)) self.selectedItems.splice(0)
            if (self.selectedItems.includes(props.index)) {
              const j = store.selectedItems.findIndex((j) => j == props.index)
              self.selectedItems.splice(j, 1)
            } else {
              self.selectedItems.push(props.index)
            }
            console.log(self.selectedItems)
          },
          anySelectAll: () => {
            self.selectedItems = self.networkDirCurrentFiles.map((_, i) => i)
          },
          anySelectInvert: () => {
            self.selectedItems = self.networkDirCurrentFiles.map((_, i) => self.selectedItems.includes(i) ? -1 : i).filter(i => i > -1)
          },
          fileDblClick: (props: { file: FioStat, index: number }) => () => {
            self.selectedItems.splice(0)
            openFile({ openPath: `${self.dir}${props.file.name}` })
          },
          folderDblClick: (props: { file: FioStat, index: number }) => async () => {
            updateDirThenCurrentFiles({ dir: `${self.dir}${props.file.name}/` })
          },
          shortcutDblClick: (props: { file: FioStat, index: number }) => async () => {
            if (props.file.type.as != "shortcut") throw new Error("Unexpected Non-Shortcut")
            self.selectedItems.splice(0)

            if (props.file.type.as != "shortcut") return
            const { prog, params } = props.file.type.shortcut
            if (prog == "@arksouthern/luna.explore")
              updateDirThenCurrentFiles({ dir: params.openPath })
            else
              openApp({ params, program: prog })
          },
        }}
      >
        {handlers => (
          <XpWindow {...props} buttons={<XpTitleButtonsNormal {...props} />} title={<>
            <img class="ml-[.062rem] mr-1 w-3.5 h-3.5" src="data:image/png;base64," alt="My Computer" draggable="false" />
            <A.TitleText class="flex-1 pointer-events-none pr-1 tracking-[.032rem] overflow-hidden whitespace-nowrap text-ellipsis">
              {(self.dir?.slice(0, -1) || "My Computer").replaceAll("/", "\\")}
            </A.TitleText>
          </>}>
            {kbdShortcutOn({ app: props, keys: ["Ctrl", "A"], callback: handlers.anySelectAll })}
            {kbdShortcutOn({ app: props, keys: ["Ctrl", "C"], callback: handlers.itemsCopy })}
            {kbdShortcutOn({ app: props, keys: ["Ctrl", "I"], callback: handlers.displaySidebar('favorites') })}
            {kbdShortcutOn({ app: props, keys: ["Ctrl", "K"], callback: handlers.displaySidebar('tree') })}
            {kbdShortcutOn({ app: props, keys: ["Ctrl", "L"], callback: handlers.displaySidebar('none') })}
            {kbdShortcutOn({ app: props, keys: ["Ctrl", "V"], callback: handlers.itemsPaste })}
            <A.Toolbars style={{ background: `linear-gradient(to right, #F2F4F2 0%, #ECE8D0 100%)` }}>
              <A.AltBar class="relative h-5 mt-0.5 ml-0.5 mb-0.5 text-xs">
                <XpBarMenu>
                  <XpBarMenuItem name="File">
                    {/* <XpBarMenuLineItem>TODO Open Command Prompt</XpBarMenuLineItem> */}
                    {/* <XpBarMenuDivider /> */}
                    <XpBarMenuLineItem disabled>TODO New</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem disabled>TODO Create Shortcut</XpBarMenuLineItem>
                    <XpBarMenuMasterItem
                      onClick={async () => {
                        await API.fileDangerouslyDelete({ filePath: `${self.dir}${self.networkDirCurrentFiles[self.selectedItems[0]].name}` })
                        await updateDirThenCurrentFiles({ dir: self.dir })
                      }}
                      title="Delete"
                      disabled={(self.selectedItems.length != 1 || !["shortcut", "file"].includes(self.networkDirCurrentFiles[self.selectedItems[0]].type.as)) as true}
                    />
                    <XpBarMenuLineItem disabled>TODO Rename</XpBarMenuLineItem>
                    <XpBarMenuMasterItem
                      onClick={() => {
                        openApp({ program: "@arksouthern/luna.explore.properties", params: { openPath: `${self.dir}${self.networkFavoritesFiles[self.selectedItems[0]].name}` } })
                      }}
                      disabled={(self.selectedItems.length != 1) as true}
                      title="Properties"
                    />
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem onClick={handlers.appClose}>Close</XpBarMenuLineItem>
                  </XpBarMenuItem>
                  <XpBarMenuItem name="Edit">
                    <XpBarMenuLineItem disabled shortcut="Ctrl+Z">TODO Undo</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem disabled shortcut="Ctrl+X">TODO Cut</XpBarMenuLineItem>
                    <XpBarMenuLineItem disabled={!self.selectedItems.length as true} onClick={handlers.itemsCopy} shortcut="Ctrl+C">Copy</XpBarMenuLineItem>
                    <XpBarMenuLineItem onClick={handlers.itemsPaste} shortcut="Ctrl+V">Paste</XpBarMenuLineItem>
                    <XpBarMenuLineItem disabled>TODO Paste Shortcut</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem disabled>TODO Copy To Folder...</XpBarMenuLineItem>
                    <XpBarMenuLineItem disabled>TODO Move To Folder...</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem
                      onClick={handlers.anySelectAll}
                      shortcut="Ctrl+A"
                    >Select All</XpBarMenuLineItem>
                    <XpBarMenuLineItem
                      onClick={handlers.anySelectInvert}
                    >Invert Selection</XpBarMenuLineItem>
                  </XpBarMenuItem>
                  <XpBarMenuItem name="View">
                    <XpBarMenuMasterItem
                      title="Toolbars"
                      xpBarMenuItemMasterList={<>
                        <XpBarMenuMasterItem
                          title="Standard Buttons"
                          indicator={self.isViewButtonsBar ? "checked" : undefined}
                          onClick={handlers.displayStandardButtonsToggle}
                        />
                        <XpBarMenuMasterItem
                          title="Address Bar"
                          indicator={self.isViewAddressBar ? "checked" : undefined}
                          onClick={handlers.displayAddressBarToggle}
                        />
                        <XpBarMenuDivider />
                        <XpBarMenuMasterItem
                          title="Lock the Toolbars"
                          indicator="checked"
                          disabled
                        />
                        <XpBarMenuMasterItem
                          title="Customize..."
                          disabled
                        />
                      </>}
                    />
                    <XpBarMenuCheckboxItem
                      boolean={self.isViewStatusBar}
                      onClick={handlers.displayStatusBarToggle}
                    >
                      Status Bar
                    </XpBarMenuCheckboxItem>

                    <XpBarMenuMasterItem
                      title="Explorer Bar"
                      xpBarMenuItemMasterList={<>
                        <XpBarMenuMasterItem
                          title="Search"
                          shortcut="Ctrl+E"
                          disabled
                        />
                        <XpBarMenuMasterItem
                          title="Favorites"
                          indicator={self.viewSidebarMx.as == "favorites" ? "checked" : undefined}
                          onClick={handlers.displaySidebar('favorites')}
                          shortcut="Ctrl+I"
                        />
                        <XpBarMenuMasterItem
                          title="History"
                          shortcut="Ctrl+H"
                          disabled
                        />
                        <XpBarMenuMasterItem
                          title="Folders"
                          indicator={self.viewSidebarMx.as == "tree" ? "checked" : undefined}
                          shortcut="Ctrl+K"
                          onClick={handlers.displaySidebar('tree')}
                        />
                        <XpBarMenuDivider />
                        <XpBarMenuMasterItem
                          title="Hidden"
                          shortcut="Ctrl+L"
                          indicator={self.viewSidebarMx.as == "none" ? "checked" : undefined}
                          onClick={handlers.displaySidebar('none')}
                        />
                      </>}
                    />
                    <XpBarMenuDivider />
                    <XpBarMenuMasterItem
                      title="Thumbnails"
                      disabled
                    />
                    <XpBarMenuMasterItem
                      title="Tiles"
                      disabled
                    />
                    <XpBarMenuMasterItem
                      title="Icons"
                      indicator={self.viewIconListMx.as == "medium" ? "radio" : undefined}
                      onClick={() => self.viewIconListMx.as = "medium"}
                    />
                    <XpBarMenuMasterItem
                      title="List"
                      indicator={self.viewIconListMx.as == "list" ? "radio" : undefined}
                      onClick={() => self.viewIconListMx.as = "list"}
                    />
                    <XpBarMenuMasterItem
                      title="Details"
                      indicator={self.viewIconListMx.as == "detail" ? "radio" : undefined}
                      onClick={() => self.viewIconListMx.as = "detail"}
                    />
                    <XpBarMenuDivider />
                    <XpBarMenuMasterItem title="Arrange Icons by" xpBarMenuItemMasterList={<>
                      <XpBarMenuMasterItem title="Name" onClick={handlers.sortsFileListToggle("name")} indicator={self.sorts.find(x => x.as == "name") ? "checked" : undefined} />
                      <XpBarMenuMasterItem title="Size" onClick={handlers.sortsFileListToggle("size")} indicator={self.sorts.find(x => x.as == "size") ? "checked" : undefined} />
                      <XpBarMenuMasterItem title="Date Modified" onClick={handlers.sortsFileListToggle("dateModified")} indicator={self.sorts.find(x => x.as == "dateModified") ? "checked" : undefined} />
                      <XpBarMenuMasterItem title="Date Created" onClick={handlers.sortsFileListToggle("dateCreated")} indicator={self.sorts.find(x => x.as == "dateCreated") ? "checked" : undefined} />
                      {/* <XpBarMenuMasterItem title="Type" />   */}
                      <XpBarMenuDivider />
                      <XpBarMenuMasterItem disabled title="TODO Show in Groups" />
                      <XpBarMenuMasterItem disabled title="Auto Arrange" />
                      <XpBarMenuMasterItem disabled title="Align to Grid" />
                    </>} />
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem disabled>TODO Choose Details...</XpBarMenuLineItem>
                    <XpBarMenuLineItem disabled>TODO Customize This Folder...</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem disabled>TODO Go To</XpBarMenuLineItem>
                    <XpBarMenuLineItem onClick={() => updateDirThenCurrentFiles({ dir: self.dir })}>Refresh</XpBarMenuLineItem>
                  </XpBarMenuItem>
                  <XpBarMenuItem name="Favorites">
                    <XpBarMenuLineItem disabled>TODO Add To Favorites...</XpBarMenuLineItem>
                    <XpBarMenuLineItem disabled>Organize Favorites...</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <For each={self.networkFavoritesFiles}>
                      {fav => (
                        <>
                          {fav.type.as != "shortcut" ? "" : (
                            <XpBarMenuMasterItem
                              title={fav.name.replaceAll(".xp.json", "")}
                              icon={
                                <img
                                  src={fav.type.shortcut.icon || getFileIconImg({ file: { name: fav.type.shortcut.params.openPath } })}
                                />
                              }
                              onClick={handlers.favClick(fav)}
                            />
                          )}
                        </>
                      )}
                    </For>
                  </XpBarMenuItem>
                  <XpBarMenuItem name="Tools">
                    <XpBarMenuLineItem>TODO Map Network Drive...</XpBarMenuLineItem>
                    <XpBarMenuLineItem>TODO Disconnect Network Drive...</XpBarMenuLineItem>
                    <XpBarMenuLineItem>TODO Syncronize...</XpBarMenuLineItem>
                    <XpBarMenuDivider />
                    <XpBarMenuLineItem>TODO Folder Options...</XpBarMenuLineItem>
                  </XpBarMenuItem>
                  <XpBarMenuItem name="Help">
                    <XpBarMenuLineItem onClick={setAbout.dialogShow}>About Ark Luna</XpBarMenuLineItem>
                  </XpBarMenuItem>
                </XpBarMenu>
              </A.AltBar>
              <A.BorderY class="border-y border-t-[#D7D2BF] border-b-white h-0" />
              <Show when={self.isViewButtonsBar}>
                <A.FunctionBar class="h-9 flex items-center text-xs px-1">
                  <A.NavFunctions class="flex items-center">
                    <button disabled class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD]">
                      <img class="w-7 h-7 -mx-[.062rem]" src="data:image/png;base64," alt="" />
                      <span class="mr-1">Back</span>
                      <div class="h-full flex items-center mx-1 before:border-x-transparent before:border-y-black before:border-[3px] before:border-b-0" />
                    </button>
                    <button disabled class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD]">
                      <img class="w-7 h-7 -mx-[.062rem]" src="data:image/png;base64," alt="" />
                      <div class="h-full flex items-center mx-1 before:border-x-transparent before:border-y-black before:border-[3px] before:border-b-0" />
                    </button>
                    <button
                      disabled={!self.dir}
                      class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD] px-0.5 py-1.5"
                      onClick={handlers.navDirUp}>
                      <img class="w-[1.375rem] h-[1.375rem]" src="data:image/png;base64," alt="" />
                      {/* <img class="w-7 h-7 -mx-[.062rem] -rotate-90" src="data:image/png;base64," alt="" /> */}
                    </button>
                    <A.Divider class='h-7 w-[.062rem] bg-black/25 mx-0.5'></A.Divider>
                    <button disabled class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD]">
                      <img
                        class="w-7 h-7 -mx-[.062rem]"
                        src="/src/assets/ico/1375.ico"
                        alt=""
                      />
                      <span class="mr-1">Search</span>
                    </button>
                    <button
                      aria-selected={self.viewSidebarMx.as == "tree"}
                      onClick={handlers.displaySidebar('tree')}
                      class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 aria-selected:border-[#8097AD] aria-selected:bg-white hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD]"
                    >
                      <img
                        class="w-7 h-7 p-0.5 -mx-[.062rem]"
                        src="/src/assets/ico/35.ico"
                        alt=""
                      />
                      <span class="mr-1">Folders</span>
                    </button>
                    <button
                      aria-selected={self.viewSidebarMx.as == "favorites"}
                      onClick={handlers.displaySidebar('favorites')}
                      class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 aria-selected:border-[#8097AD] aria-selected:bg-white hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD]"
                    >
                      <img
                        class="w-7 h-7 p-0.5 -mx-[.062rem]"
                        src="/src/assets/shell32/44.ico"
                        alt=""
                      />
                      <span class="mr-1">Favorites</span>
                    </button>
                    <A.Divider class='h-7 w-[.062rem] bg-black/25 mx-0.5'></A.Divider>
                    <XpDropDownMenu
                      placement='bottom-start'
                      xpDropDownTrigger={(
                        <XpDropDownTrigger disabled={!self.dir}>
                          <button
                            disabled={!self.dir}
                            class="h-9 disabled:grayscale flex items-center rounded-sm border border-black/0 hover:border-[#CECEC4] active:text-white active:bg-[#E3E3DD] pl-1 pr-0.5 py-1.5"
                          >
                            <img class="w-5 h-5 -mx-[.062rem]" src="/src/assets/ico/160.ico" alt="" />
                            <div class="h-full flex items-center mx-1 before:border-x-transparent before:border-y-black before:border-[3px] before:border-b-0" />
                          </button>
                        </XpDropDownTrigger>
                      )}
                      xpDropDownItemList={<>
                        <For each={["detail", "medium", "list"] as const}>
                          {x => (
                            <XpDropDownItem
                              title={<span class="capitalize">{x}</span>}
                              indicator={self.viewIconListMx.as == x ? "radio" : undefined}
                              onClick={() => self.viewIconListMx.as = x}
                            />
                          )}
                        </For>
                      </>}
                    />
                  </A.NavFunctions>
                </A.FunctionBar>
                <A.BorderY class="h-0 border-y border-t-[#D7D2BF] border-b-white" />
              </Show>
              <Show when={self.isViewAddressBar}>
                <A.AddressBar class="text-xs flex items-center pr-0.5 h-5" style={{ "box-shadow": `#CCCABD 0px -1.25px 1px 0px inset` }}>
                  <div class="p-1.5 text-black/50 leading-[100%]">Address</div>
                  <A.ContextContainer class="border border-[#859CB6] bg-white relative flex h-full flex-1 items-center gap-1">
                    <img class="w-3.5 h-3.5" src="data:image/png;base64," alt="ie" />
                    <input
                      class="flex-1 h-full outline-none"
                      onChange={e => self.dir = e.currentTarget.value}
                      onKeyDown={async e => {
                        if (e.key != 'Enter') return
                        updateDirThenCurrentFiles({ dir: e.currentTarget.value })
                      }}
                      value={self.dir || "My Computer"}
                    />
                  </A.ContextContainer>
                  <A.GoButton onClick={async () => {
                    await API.folderRead({ dir: self.dir })
                  }} class="hover:brightness-110 hover:bg-[#F6F6F2] border-x border-black/0 hover:border-[#CECEC4] flex items-center h-full relative pr-1.5 mr-3.5 ml-0.5 pl-0.5">
                    <img class="h-[95%] mr-1 border border-white rounded-sm" src="data:image/png;base64," alt="go" />
                    <div>Go</div>
                  </A.GoButton>
                </A.AddressBar>
              </Show>
            </A.Toolbars>
            <A.MainContent class="border-l border-b border-l-[rgb(237,237,229)] border-b-white flex-1 block text-[.75rem] overflow-y-auto bg-white" style={{}}>
              <MatchAs over={self.viewSidebarMx} match={{
                none: () => <></>,
                prompts: x => (
                  <A.ContentSidebar
                    class="overflow-auto [flex-basis:0] float-start w-1/4 max-w-56 min-w-48 p-2.5 space-y-3 sticky h-full top-0"
                    style={{ background: `linear-gradient(#82A0E0, #6674D0)` }}>
                    <XpAsideAccordion title="System Tasks">
                      <XpAsideAccordionItem
                        icon="/src/assets/shell32/271.ico"
                        children="Add or remove programs"
                        onClick={() => openApp({ program: "@arksouthern/luna.stellar", params: { openPath: "" } })}
                      />
                      <XpAsideAccordionItem
                        icon="/src/assets/ico/182.ico"
                        children="Change a setting"
                        disabled
                        onClick={() => { }}
                      />
                    </XpAsideAccordion>
                    <Show when={self.dir}>
                      <XpAsideAccordion title="File and Folder Tasks">
                        <Show when={self.selectedItems.length} fallback={
                          <>
                            <XpAsideAccordionItem
                              icon="/src/assets/shell32/319.ico"
                              children="Make a new folder"
                              disabled
                              onClick={() => { }}
                            />
                            <XpAsideAccordionItem
                              icon="/src/assets/vs0710/Objects2012/png_format/WinVista/cmd.png"
                              children="Open this folder in Command Prompt"
                              onClick={() => {
                                openApp({ program: "@arksouthern/luna.shell", params: { openPath: `cd ${self.dir}` } })
                              }}
                            />
                            <XpAsideAccordionItem
                              icon="/src/assets/shell32/237.ico"
                              children="Add this folder to your Music Library"
                              onClick={async () => {
                                await apiCreateShortcut({
                                  file: {
                                    type: { as: "folder" },
                                    name: self.dir.split("/").pop().split("\\").pop()
                                  } as any,
                                  openPath: self.dir,
                                  saveLocation: "Music"
                                })
                              }}
                            />
                            <XpAsideAccordionItem
                              icon="/src/assets/shell32/314.ico"
                              children="Add this folder to your Game Library"
                              onClick={async () => {
                                await apiCreateShortcut({
                                  file: {
                                    type: { as: "folder" },
                                    name: self.dir.split("/").pop().split("\\").pop()
                                  } as any,
                                  openPath: self.dir,
                                  saveLocation: "Games"
                                })
                              }}
                            />
                            <XpAsideAccordionItem
                              icon="/src/assets/shell32/173.ico"
                              children="Favorite this folder"
                              onClick={async () => {
                                await apiCreateShortcut({
                                  file: {
                                    type: { as: "folder" },
                                    name: self.dir.split("/").filter(Boolean).pop().split("\\").filter(Boolean).pop()
                                  } as any,
                                  openPath: self.dir,
                                  saveLocation: "Favorites"
                                })
                              }}
                            />
                          </>
                        }>
                          <XpAsideAccordionItem
                            icon="/src/assets/shell32/35.ico"
                            onClick={async () => {
                              const file = self.networkDirCurrentFiles[self.selectedItems[0]]
                              await apiCreateShortcut({ file, openPath: `${self.dir}${file.name}`, saveLocation: "Desktop" })
                            }}
                            children="Create Desktop shortcut"
                          />
                          <XpAsideAccordionItem
                            disabled
                            icon="/src/assets/ico/856.ico"
                            onClick={() => { }}
                            children="Rename this file"
                          />
                          <XpAsideAccordionItem
                            disabled
                            icon="/src/assets/ico/475.ico"
                            onClick={() => { }}
                            children="Move this file"
                          />
                          <XpAsideAccordionItem
                            disabled
                            icon="/src/assets/ico/541.ico"
                            onClick={() => { }}
                            children="Copy this file"
                          />
                          <XpAsideAccordionItem
                            disabled
                            icon="/src/assets/ico/848.ico"
                            onClick={() => { }}
                            children="Delete this file"
                          />
                        </Show>
                      </XpAsideAccordion>
                    </Show>
                    <XpAsideAccordion title="Other Places">
                      <XpAsideAccordionItem
                        disabled
                        icon="/src/assets/ico/170.ico"
                        onClick={() => { }}
                        children="Recent Documents"
                      />
                      <XpAsideAccordionItem
                        icon="/src/assets/ico/796.ico"
                        onClick={async () => {
                          updateDirThenCurrentFiles({ dir: "../data/Documents/" })
                        }}
                        children="My Documents"
                      />
                      <XpAsideAccordionItem
                        icon="/src/assets/ico/123.ico"
                        onClick={async () => {
                          self.networkDirCurrentFiles = [] // await API.fileExplorerReadFolder({ dir: "/" })
                          self.dir = ""
                        }}
                        children="My Computer"
                      />
                    </XpAsideAccordion>
                    <XpAsideAccordion title="Details">
                      <div class="leading-3 text-[.62rem]">
                        {
                          !self.selectedItems.length ?
                            <>
                              <A.Title class="font-semibold">{self.dir.split("/").filter(Boolean).pop() || "My Computer"}</A.Title>
                              <A.Type>File Folder</A.Type>
                              <A.Spacer class="h-0" />
                            </> :
                            self.selectedItems.length > 1 ?
                              <>
                                {self.selectedItems.length} items selected.
                              </> :
                              <>
                                <Variables item={self.networkDirCurrentFiles[self.selectedItems[0]]}>
                                  {props => (
                                    <>
                                      <A.Title class="font-semibold">{props.item.name}</A.Title>
                                      <A.Type>
                                        <MatchAs over={props.item.type}
                                          match={{
                                            file: () => <>{props.item.name.split(".").pop()?.toUpperCase()} File</>,
                                            folder: () => <>File Folder</>,
                                            locked: () => <>Locked File</>,
                                            shortcut: () => <>Shortcut File</>
                                          }}
                                        />
                                      </A.Type>
                                      <A.Spacer class="h-1" />
                                      <A.Type>Date Modified: {new Date(props.item.mtimeMs).toLocaleString()}</A.Type>
                                      <A.Spacer class="h-1" />
                                      <A.Type>Size: {humanFileSize(props.item.size)}</A.Type>
                                    </>
                                  )}
                                </Variables>
                              </>
                        }
                      </div>
                    </XpAsideAccordion>
                  </A.ContentSidebar>
                ),
                tree: x => (
                  <A.TreeSidebar class="float-start w-1/4 max-w-64 min-w-48 pr-1 sticky h-full flex flex-col top-0 bg-[linear-gradient(90deg,#F3F5F7,#EBE9DA)]">
                    <A.Title class="px-1 py-0.5 text-[.62rem] flex">
                      <span class="flex-1">
                        Folders
                      </span>
                      <A.Close
                        class="w-4 h-4 leading-[1.2rem] flex items-center justify-center hover:bg-white/30 rounded-sm border-black/30 hover:border"
                        onClick={() => self.viewSidebarMx = { as: "prompts" }}
                      >
                        x
                      </A.Close>
                    </A.Title>
                    <A.Inner class="bg-white pl-1.5 overflow-auto flex-1 [flex-basis:0] border-t border-[#ABA89B]">
                      <DirTree
                        path="/"
                        dirTree={self.networkTreeDir}
                      />
                    </A.Inner>
                  </A.TreeSidebar>
                ),
                favorites: x => (
                  <A.TreeSidebar class="float-start w-1/4 max-w-64 min-w-48 pr-1 sticky h-full flex flex-col top-0 bg-[linear-gradient(90deg,#F3F5F7,#EBE9DA)]">
                    <A.Title class="px-1 py-0.5 text-[.62rem] flex">
                      <span class="flex-1">
                        Favorites
                      </span>
                      <A.Close
                        class="w-4 h-4 leading-[1.2rem] flex items-center justify-center hover:bg-white/30 rounded-sm border-black/30 hover:border"
                        onClick={() => self.viewSidebarMx = { as: "prompts" }}
                      >
                        x
                      </A.Close>
                    </A.Title>
                    <A.Inner class="bg-white px-0.5 overflow-auto text-xs flex-1 [flex-basis:0] border-t border-[#ABA89B]">
                      <For each={self.networkFavoritesFiles}>
                        {file => (
                          <>
                            {file.type.as != "shortcut" ? "" : (
                              <A.File
                                class="flex gap-0.5 items-center hover:underline hover:text-[#4069BF] active:bg-[#4069BF] active:text-white aria-selected:bg-[#EBE9DA]"
                                aria-selected={file.type.shortcut.params.openPath == self.dir}
                                onClick={handlers.favClick(file)}
                              >
                                <img class="w-5 h-5" src={file.type.shortcut.icon || getFileIconImg({ file: { name: file.type.shortcut.params.openPath } })} />
                                <p>
                                  <span class="hover:cursor-pointer">
                                    {file.name.replaceAll(".xp.json", "")}
                                  </span>
                                </p>
                              </A.File>
                            )}
                          </>
                        )}
                      </For>
                    </A.Inner>
                  </A.TreeSidebar>
                )
              }} />
              <div class="flex-1 bg-white outline-none overflow-x-auto  h-full resize-none">
                <Show
                  when={self.networkDirCurrentFiles.length}
                  fallback={
                    <A.DiskDrives>
                      <A.BorderT class="border-t border-[#96abff]" />
                      <A.FileSection>
                        <A.FileSectionHeader class='font-bold pl-3'>Files Stored on This Computer</A.FileSectionHeader>
                        <A.FileSectionDivider class='h-0.5 bg-gradient-to-r from-[rgb(112,191,255)] to-white w-2/3 mb-4 ' />
                        <A.DiskDrive class='flex items-center' onDblClick={async () => {
                          updateDirThenCurrentFiles({ dir: "../data/" })
                        }}>
                          <img class='w-11 h-11 mr-1 ml-4' src="/src/assets/shell32/259.ico" />
                          <p class='tracking-tighter'>My Home Folder</p>
                        </A.DiskDrive>
                      </A.FileSection>
                      <A.SpaceY class="h-5" />
                      <A.FileSection>
                        <A.FileSectionHeader class='font-bold pl-3'>Hard Disk Drives</A.FileSectionHeader>
                        <A.FileSectionDivider class='h-0.5 bg-gradient-to-r from-[rgb(112,191,255)] to-white w-2/3 mb-4 ' />
                        <A.DiskDrive onDblClick={async () => {
                          updateDirThenCurrentFiles({ dir: "/" })
                        }} class='flex items-center'>
                          <img class='w-11 h-11 mr-1 ml-4' src="/src/assets/ico/73.ico" />
                          <p class='tracking-tighter'>Host Disk</p>
                        </A.DiskDrive>
                      </A.FileSection>
                    </A.DiskDrives>
                  }
                >
                  <MatchAs over={self.viewIconListMx} match={{
                    detail: x => (
                      <A.FileSection class='flex flex-col text-xs'>
                        <div class='flex tabular-nums'>
                          <div class='justify-start flex-1 ' >
                            <div class='text-xs pb-0.5 flex flex-1 pl-4 gap-2 bg-[linear-gradient(90deg,#EBEADC,#EDEADA)]' style={{ "box-shadow": `#CAC7BA 0px -1.25px 2px 0px inset` }}>
                              <p onClick={handlers.sortsFileListToggle('name')} class='flex-1 max-w-64 flex relative'>Name
                                <Show when={self.sorts.find((s) => s.as == "name")}>
                                  {self.sorts[self.sorts.findIndex((s) => s.as == "name")].direction == "up"
                                    ? <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-bottom": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>
                                    : <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-top": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>}
                                </Show>
                              </p>
                              <A.BorderContainer class="pt-0.5">
                                <A.Border class="border-r-white border-l-[#C7C5B4] border-x h-full" />
                              </A.BorderContainer>
                              <p onClick={handlers.sortsFileListToggle('size')} class='min-w-32 text-right flex'>Size
                                <Show when={self.sorts.find((s) => s.as == "size")}>
                                  {self.sorts[self.sorts.findIndex((s) => s.as == "size")].direction == "up"
                                    ? <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-bottom": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>
                                    : <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-top": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>}
                                </Show>
                              </p>
                              <A.BorderContainer class="pt-0.5">
                                <A.Border class="border-r-white border-l-[#C7C5B4] border-x h-full" />
                              </A.BorderContainer>
                              <p onClick={handlers.sortsFileListToggle('dateModified')} class='min-w-32 text-left flex'>Date Modified
                                <Show when={self.sorts.find((s) => s.as == "dateModified")}>
                                  {self.sorts[self.sorts.findIndex((s) => s.as == "dateModified")].direction == "up"
                                    ? <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-bottom": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>
                                    : <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-top": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>}
                                </Show>
                              </p>
                              <A.BorderContainer class="pt-0.5">
                                <A.Border class="border-r-white border-l-[#C7C5B4] border-x h-full" />
                              </A.BorderContainer>
                              <p onClick={handlers.sortsFileListToggle('dateCreated')} class='min-w-32 text-left flex'>Created
                                <Show when={self.sorts.find((s) => s.as == "dateCreated")}>
                                  {self.sorts[self.sorts.findIndex((s) => s.as == "dateCreated")].direction == "up"
                                    ? <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-bottom": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>
                                    : <div style={{ "border-left": "5px solid transparent", "border-right": "5px solid transparent", "border-top": "5px solid #aca899" }} class="w-0 h-0 mt-1.5 ml-1"></div>}
                                </Show>
                              </p>
                            </div>

                            <For
                              each={self.getSortedFiles}
                              children={(file, i) => (
                                <FileHoverPlusContext file={file} openPath={`${self.dir}${file.name}`}>
                                  <MatchAs over={file.type} match={{
                                    file: x => (
                                      <A.DiskDrive
                                        class='flex items-center'
                                        onDblClick={handlers.fileDblClick({ file, get index() { return i() } })}
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <div class='min-w-0 w-4 py-[.062rem]'>
                                          <img aria-hidden={file.name.startsWith(".")} class='object-contain w-4 h-4 mr-1 ml-px aria-hidden:opacity-75' src={getFileIconImg({ file })} />
                                        </div>
                                        <div aria-selected={self.selectedItems.includes(i())} class='group flex flex-1 gap-2'>
                                          <p
                                            aria-selected={self.selectedItems.includes(i())}
                                            class='tracking-tighter text-nowrap overflow-hidden text-ellipsis max-w-64 flex-1'
                                          >
                                            <span class="group-aria-selected:bg-[#4069BF] group-aria-selected:text-white group-aria-selected:border border-dotted pr-1 border-[#C5974B]">
                                              {file.name}
                                            </span>
                                          </p>
                                          <A.BorderInvis class="border-x border-transparent" />
                                          <p
                                            class='tracking-tighter min-w-32 text-right'
                                          >
                                            {file.size && humanFileSize(file.size)}
                                          </p>
                                          <A.BorderInvis class="border-x border-transparent" />
                                          <p
                                            class='tracking-tighter min-w-32 text-left'
                                          >
                                            {file.ctimeMs ? new Date(file.ctimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' }) : ""}
                                          </p>
                                          <A.BorderInvis class="border-x border-transparent" />
                                          <p
                                            class='tracking-tighter min-w-32 text-left'
                                          >
                                            {file.birthtimeMs ? new Date(file.birthtimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' }) : ""}
                                          </p>
                                        </div>
                                      </A.DiskDrive>
                                    ),
                                    folder: x => (
                                      <>
                                        <A.DiskDrive class='flex items-center'
                                          onDblClick={handlers.folderDblClick({ file, get index() { return i() } })}
                                          onClick={handlers.anySelect({ file, get index() { return i() } })}
                                        >
                                          <div class='min-w-0 w-4 py-[.062rem]'>
                                            <img aria-hidden={file.name.startsWith(".")} class='aria-hidden:opacity-75 w-4 h-4 mr-1 ml-px' src="data:image/png;base64," alt="folder" />
                                          </div>
                                          <div aria-selected={self.selectedItems.includes(i())} class='group flex flex-1 gap-2'>
                                            <p
                                              aria-selected={self.selectedItems.includes(i())}
                                              class='tracking-tighter text-nowrap overflow-hidden text-ellipsis max-w-64 flex-1'
                                            >
                                              <span class="group-aria-selected:bg-[#4069BF] group-aria-selected:text-white group-aria-selected:border border-dotted pr-1 border-[#C5974B]">
                                                {file.name}
                                              </span>
                                            </p>
                                            <A.BorderInvis class="border-x border-transparent" />
                                            <p
                                              class='tracking-tighter min-w-32 text-right'></p>
                                            <A.BorderInvis class="border-x border-transparent" />
                                            <p
                                              class='tracking-tighter min-w-32 text-left'
                                            >
                                              {file.ctimeMs ? new Date(file.ctimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' }) : ""}
                                            </p>
                                            <A.BorderInvis class="border-x border-transparent" />
                                            <p
                                              class='tracking-tighter min-w-32 text-left'
                                            >
                                              {file.birthtimeMs ? new Date(file.birthtimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' }) : ""}
                                            </p>
                                          </div>
                                        </A.DiskDrive>
                                      </>
                                    ),
                                    locked: x => (
                                      <A.DiskDrive class='flex items-center opacity-60'
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <div class='min-w-0 w-4 py-[.062rem]'>
                                          <img aria-hidden={file.name.startsWith(".")} class='aria-hidden:opacity-75 w-4 h-4 mr-1 ml-px' src="/src/assets/ico/368.ico" alt="Locked File" />
                                        </div>
                                        <div aria-selected={self.selectedItems.includes(i())} class='aria-selected:bg-[#4069BF] aria-selected:text-white flex flex-1 gap-2'>
                                          <p aria-selected={self.selectedItems.includes(i())} class='text-nowrap aria-selected:bg-[#4069BF] tracking-tighter overflow-hidden text-ellipsis max-w-64 flex-1'>{file.name}</p>
                                          <p class='tracking-tighter min-w-32 text-right'></p>
                                          <p class='tracking-tighter min-w-32 text-right'></p>
                                          <p class='tracking-tighter min-w-32 text-right'></p>
                                        </div>
                                      </A.DiskDrive>
                                    ),
                                    shortcut: x => (
                                      <A.DiskDrive
                                        class='flex items-center'
                                        onDblClick={handlers.shortcutDblClick({ file, get index() { return i() } })}
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <div class='min-w-0 w-4 py-[.062rem]'>
                                          <img aria-hidden={file.name.startsWith(".")} class='object-contain w-4 h-4 mr-1 ml-px aria-hidden:opacity-75' src={x.shortcut.icon || getFileIconImg({ file: { name: x.shortcut.params.openPath } })} />
                                        </div>
                                        <div aria-selected={self.selectedItems.includes(i())} class='group flex flex-1 gap-2'>
                                          <p
                                            aria-selected={self.selectedItems.includes(i())}
                                            class='tracking-tighter text-nowrap overflow-hidden text-ellipsis max-w-64 flex-1'
                                          >
                                            <span class="group-aria-selected:bg-[#4069BF] group-aria-selected:text-white group-aria-selected:border border-dotted pr-1 border-[#C5974B]">
                                              {file.name.replaceAll(".xp.json", "")}
                                            </span>
                                          </p>
                                          <A.BorderInvis class="border-x border-transparent" />
                                          <p
                                            class='tracking-tighter min-w-32 text-right'
                                          >
                                            {file.size && humanFileSize(file.size)}
                                          </p>
                                          <A.BorderInvis class="border-x border-transparent" />
                                          <p
                                            class='tracking-tighter min-w-32 text-left'
                                          >
                                            {file.ctimeMs ? new Date(file.ctimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' }) : ""}
                                          </p>
                                          <A.BorderInvis class="border-x border-transparent" />
                                          <p
                                            class='tracking-tighter min-w-32 text-left'
                                          >
                                            {file.birthtimeMs ? new Date(file.birthtimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' }) : ""}
                                          </p>
                                        </div>
                                      </A.DiskDrive>
                                    )
                                  }} />
                                </FileHoverPlusContext>
                              )}
                            />
                          </div>
                        </div>
                      </A.FileSection>
                    ),

                    medium: x => (
                      <>
                        <A.BorderT class="border-t border-[#96abff]" />
                        <A.FileSection class='flex flex-col text-xs'>
                          <div class='flex pt-2'>
                            <div class='flex-1 gap-0.5 justify-start text-[.62rem]'>
                              <For
                                each={self.getSortedFiles}
                                children={(file, i) => (
                                  <FileHoverPlusContext file={file} openPath={`${self.dir}${file.name}`}>
                                    <A.DiskDrive
                                      aria-hidden={file.name.startsWith(".")}
                                      class='aria-hidden:opacity-70 float-left text-center w-16 h-16 justify-start mx-0.5 overflow-hidden flex flex-col items-center'
                                      onDblClick={
                                        mx(file.type)({
                                          file: () => handlers.fileDblClick({ file, get index() { return i() } }),
                                          folder: () => handlers.folderDblClick({ file, get index() { return i() } }),
                                          locked: () => () => { },
                                          shortcut: () => handlers.shortcutDblClick({ file, get index() { return i() } })
                                        })(file.type)
                                      }
                                      onClick={handlers.anySelect({ file, get index() { return i() } })}
                                    >
                                      <img
                                        aria-hidden={file.name.startsWith(".")}
                                        class='object-contain w-8 h-8 mx-auto aria-hidden:opacity-75'
                                        src={
                                          mx(file.type)({
                                            file: () => getFileIconImg({ file }),
                                            folder: () => "data:image/png;base64,",
                                            locked: () => "data:image/png;base64,",
                                            shortcut: x => x.shortcut.icon || getFileIconImg({ file: { name: x.shortcut.params.openPath } })
                                          })(file.type)
                                        }
                                      />
                                      <p
                                        aria-selected={self.selectedItems.includes(i())}
                                        class='aria-selected:bg-[#4069BF] aria-selected:text-white tracking-tighter overflow-hidden text-ellipsis px-0.5 max-w-16'
                                      >
                                        {file.name}
                                      </p>
                                    </A.DiskDrive>
                                  </FileHoverPlusContext>
                                )}
                              />
                            </div>
                          </div>
                        </A.FileSection>
                      </>
                    ),

                    list: x => (
                      <A.FileSectionListStyle class="h-full w-full overflow-x-auto text-xs">
                        <A.DirectList class="flex flex-col flex-wrap content-start h-full gap-x-1">
                          <For each={self.getSortedFiles}>
                            {(file, i) => (
                              <FileHoverPlusContext file={file} openPath={`${self.dir}${file.name}`}>
                                <div>
                                  <MatchAs over={file.type} match={{
                                    file: () => (
                                      <div class="flex w-32 items-center gap-px"
                                        onDblClick={handlers.fileDblClick({ file, get index() { return i() } })}
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <img aria-hidden={file.name.startsWith(".")} class='aria-hidden:opacity-75 object-contain w-4 h-w-4' src={getFileIconImg({ file })} />
                                        <p
                                          aria-selected={self.selectedItems.includes(i())}
                                          class='aria-selected:bg-[#4069BF] aria-selected:text-white tracking-tighter overflow-hidden text-ellipsis text-nowrap'
                                        >
                                          {file.name}
                                        </p>
                                      </div>
                                    ),
                                    shortcut: (x) => (
                                      <div class="flex w-32 items-center gap-px"
                                        onDblClick={handlers.shortcutDblClick({ file, get index() { return i() } })}
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <img
                                          aria-hidden={file.name.startsWith(".")}
                                          class='aria-hidden:opacity-75 object-contain w-4 h-w-4'
                                          src={x.shortcut.icon || getFileIconImg({ file: { name: x.shortcut.params.openPath } })}
                                        />
                                        <p
                                          aria-selected={self.selectedItems.includes(i())}
                                          class='aria-selected:bg-[#4069BF] aria-selected:text-white tracking-tighter overflow-hidden text-ellipsis text-nowrap'
                                        >
                                          {file.name.replace(".xp.json", "")}
                                        </p>
                                      </div>
                                    ),
                                    folder: () => (
                                      <div class="flex w-32 items-center gap-px"
                                        onDblClick={handlers.folderDblClick({ file, get index() { return i() } })}
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <img
                                          aria-selected={self.selectedItems.includes(i())}
                                          aria-hidden={file.name.startsWith(".")}
                                          class='aria-hidden:opacity-75 aria-selected:opacity-50 object-contain w-4 h-w-4'
                                          src="data:image/png;base64,"
                                          alt="folder"
                                        />
                                        <p
                                          aria-selected={self.selectedItems.includes(i())}
                                          class="aria-selected:bg-[#4069BF] aria-selected:text-white tracking-tighter overflow-hidden text-ellipsis text-nowrap"
                                        >{file.name}</p>
                                      </div>
                                    ),
                                    locked: () => (
                                      <div class="flex w-32 items-center gap-px"
                                        onDblClick={handlers.folderDblClick({ file, get index() { return i() } })}
                                        onClick={handlers.anySelect({ file, get index() { return i() } })}
                                      >
                                        <img
                                          aria-selected={self.selectedItems.includes(i())}
                                          aria-hidden={file.name.startsWith(".")}
                                          class='aria-hidden:opacity-75 aria-selected:opacity-50 object-contain w-4 h-w-4'
                                          src="data:image/png;base64,"
                                          alt="folder"
                                        />
                                        <p
                                          aria-selected={self.selectedItems.includes(i())}
                                          class="aria-selected:bg-[#4069BF] aria-selected:text-white tracking-tighter overflow-hidden text-ellipsis text-nowrap"
                                        >{file.name}</p>
                                      </div>
                                    )
                                  }} />
                                </div>
                              </FileHoverPlusContext>
                            )}
                          </For>
                        </A.DirectList>
                      </A.FileSectionListStyle>
                    ),
                  }} />
                </Show>
              </div>
            </A.MainContent>
            {self.isViewStatusBar && (
              <A.StatusBar class="flex text-xs items-center pt-1 pb-0.5 bg-[#EBE9DA]" style={{ "box-shadow": `rgba(45, 45, 45, .4) 0 3px 3px -1px inset` }}>
                <A.Details class="flex-1 pl-0.5">
                  {
                    !self.selectedItems.length ?
                      <>
                        {self.networkDirCurrentFiles.length} objects
                      </> :
                      self.selectedItems.length > 1 ?
                        <>
                          {self.selectedItems.length} objects selected
                        </> :
                        <>
                          {self.networkDirCurrentFiles[self.selectedItems[0]].name}
                        </>
                  }
                </A.Details>
                <A.Border class="w-px h-full mx-2 border-l border-r border-l-[#C4C1B1] border-r-[#FFFFFF]" />
                <A.Size class="w-24">
                  {self.selectedItems.length == 1 && humanFileSize(self.networkDirCurrentFiles[self.selectedItems[0]].size || 0)}
                </A.Size>
                <A.Border class="w-px h-full mx-2 border-l border-r border-l-[#C4C1B1] border-r-[#FFFFFF]" />
                <A.Home class="w-24">
                  My Computer
                </A.Home>
              </A.StatusBar>
            )}
          </XpWindow>
        )}
      </HandleSet>
    </A.FileExplorer>
  )

  function FileHoverPlusContext(props: { children: any, file: FioStat, openPath: string }) {
    return (
      <XpRightClickMenu xpRightClickMenuItemList={<>
        <XpRightClickMenuItem onClick={() => props.file.type.as == "file" ? openFile(props) : openApp({ program: "@arksouthern/luna.explore", params: props })} title={<b>Open</b>} />
        {props.file.type.as == "folder" && (
          <>
            <XpRightClickMenuItem disabled title="TODO Explore" />
          </>
        )}
        {/* <XpRightClickMenuItem title="? Edit" /> */}
        {props.file.type.as == "shortcut" && (
          <>
            <XpRightClickMenuItem
              disabled
              title="TODO Open file location"
              onClick={() => {
                // if (props.file.type.as != "shortcut") return
                // console.log(props)
                // const openPath = props.file.type.shortcut.params.openPath || ""
                // updateDirThenCurrentFiles({ dir: openPath })
              }}
            />
            <XpRightClickMenuItem disabled title="TODO Pin to Start Menu" />
          </>
        )}
        <XpRightClickMenuDivider />
        <XpRightClickMenuItem title="Send To" xpRightClickMenuItemList={<>
          <XpRightClickMenuItem title="Desktop" onClick={() => {
            apiCreateShortcut({ file: props.file, saveLocation: "Desktop", openPath: props.openPath })
          }} />
          <XpRightClickMenuItem title="My Documents" onClick={() => {
            apiCreateShortcut({ file: props.file, saveLocation: "Documents", openPath: props.openPath })
          }} />
        </>} />
        <XpRightClickMenuItem title="? Open With" xpRightClickMenuItemList={<>
          <XpRightClickMenuItem title="TODO" />
          <XpRightClickMenuDivider />
          <XpRightClickMenuItem title="Choose Program..." />
        </>} />
        <XpRightClickMenuDivider />
        <XpRightClickMenuItem disabled title="TODO Cut" />
        <XpRightClickMenuItem onClick={() => clipboardFsCopy({ openPathList: [props.openPath] })} title="Copy" />
        <XpRightClickMenuDivider />
        <XpRightClickMenuItem disabled title="TODO Create Shortcut" />
        <XpRightClickMenuItem disabled title="TODO Delete" />
        <XpRightClickMenuItem disabled title="TODO Rename" />
        <XpRightClickMenuDivider />
        <XpRightClickMenuItem title="Properties" onClick={() => {
          openApp({ program: "@arksouthern/luna.explore.properties", params: { openPath: props.openPath } })
        }} />
      </>}>
        <HoverCard hideWhenDetached placement="bottom-start">
          <HoverCardTrigger>{props.children}</HoverCardTrigger>
          <HoverCardContent
            class="z-10 bg-[#ffffe4] -space-y-1 fill-mode-forwards animate-out fade-out duration-250 data-[expanded]:fill-mode-none data-[expanded]:animate-in data-[expanded]:fade-in data-[expanded]:duration-250 border border-black text-black rounded-none py-0.5 px-1 text-xs w-max"
          >
            <Switch>
              <SwitchMatch when={props.file.type.as == "locked"}>
                <p>Type: <span class="uppercase">{props.file.name.split(".").pop() || props.file.name || "UNKNOWN"}</span></p>
              </SwitchMatch>
              <SwitchMatch when={props.file.type.as == "folder"}>
                {
                  props.file.size
                    ? <>
                      <p>Size: <span>{humanFileSize(props.file.size)}</span></p>
                      {props.file.birthtimeMs && <p>Date Created: {new Date(props.file.birthtimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' })}</p>}
                      {props.file.ctimeMs && <p>Date Modified: {new Date(props.file.ctimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' })}</p>}
                    </>
                    : <p>Folder is Empty</p>
                }
              </SwitchMatch>
              <SwitchMatch when={true}>
                <p>Type: <span class="uppercase">{props.file.name.split(".").pop() || props.file.name || "UNKNOWN"}</span></p>
                {props.file.birthtimeMs && <p>Date Created: {new Date(props.file.birthtimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' })}</p>}
                {props.file.ctimeMs && <p>Date Modified: {new Date(props.file.ctimeMs).toLocaleString(undefined, { day: 'numeric', month: 'numeric', year: 'numeric', hour12: true, hour: 'numeric', minute: '2-digit' })}</p>}
                {props.file.size && <p>Size: {humanFileSize(props.file.size)}</p>}
              </SwitchMatch>
            </Switch>
          </HoverCardContent>
        </HoverCard>
      </XpRightClickMenu>
    )
  }

}

function getFileIconImg(props: { file: { name: string } }) {
  const { name } = props.file
  const ico = (id: number) => `/src/assets/ico/${id}.ico`
  if (name.startsWith("."))
    return ico(523)
  const opener = queryFileOpener({openPath: name})
  if (opener.as == "opener") return opener.opener.icon
  return ico(1070)
}

// https://stackoverflow.com/questions/10420352/converting-file-size-in-bytes-to-human-readable-string
function humanFileSize(size: number) {
  const i = Math.floor(Math.log(size) / Math.log(1024))
  return `${(size / 1024 ** i).toFixed(2)} ${['B', 'KB', 'MB', 'GB', 'TB'][i]}`.replace(".00", "")
}

async function apiCreateShortcut(props: { file: FioStat, openPath: string, saveLocation: SaveLocation }) {
  let sc = { prog: "@arksouthern/luna.explore", icon: "/src/assets/ico/23.ico" }
  // @ts-ignore
  if (props.file.type.as == "folder");
  // @ts-ignore
  else if (props.file.type.as == "locked");
  else {
    const [ext, info] = Object.entries(store.desktopOpener.extensions).find(x => props.file.name.endsWith(`.${x}`)) || [null, null]
    if (ext && info) {
      sc = info
    }
  }
  await API.shortcutCreateBasic({ progName: props.file.name, saveLocation: props.saveLocation, params: { openPath: props.openPath }, ...sc })
}
